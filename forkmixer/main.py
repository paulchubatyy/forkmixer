...
class TypeMixer(_.with_metaclass(TypeMixerMeta)):
    ...
    def make_fabric(self, scheme, field_name=None, fake=None, kwargs=None):  # noqa
        ...
        if not fab:
            return partial(
                type(self)(
                    scheme, mixer=self.__mixer, fake=self.__fake, factory=self.__factory
                ).blend,
                **kwargs,
            )
...
class ProxyMixer:
    ...
    def blend(self, scheme, **values):
        ...
        for _ in range(self.count):
            result.append(self.mixer.blend(scheme, **values))
        return result
...
class Mixer(_.with_metaclass(_MetaMixer)):
    ...
    def __init__(
        ...
        **params,
        ...
        self.__init_params__(
            fake=fake, loglevel=loglevel, silence=silence, locale=locale
        )
    ...
    def __getattr__(self, name):
        if name in ["f", "g", "fake", "random", "mix", "select"]:
            warnings.warn(
                '"mixer.%s" is depricated, use "mixer.%s" instead.'
                % (name, name.upper()),
                stacklevel=2,
            )
            name = name.upper()
